...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".pgc-calendar-wrapper"), function (calendarWrapper, calendarCounter) {
        ...
        config = Object.assign(config, {
			...
			const locations = info.event.extendedProps.location.split(',').map(loc => loc.trim());
    			const locationLinks = locations.map(location => {
        			const locationURL = location.startsWith("http") ? location : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}`;
        		return `<a target="_blank" href="${locationURL}">${location}</a>`;
    			});

    			texts.push(`
			<div class="pgc-popup-row pgc-event-location">
			<div class="pgc-popup-row-icon"><span class="dashicons dashicons-location"></span></div>
			<div class="pgc-popup-row-value">${locationLinks.join(', ')}</div>
        			</div>
    			`);
		    }
